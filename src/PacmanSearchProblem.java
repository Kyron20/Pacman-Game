...
class ManhattanDistanceHeuristic implements SearchHeuristic<PacmanPositionSearchState, PacmanAction> {
    ...
    @Override
    public Double value(PacmanPositionSearchState state,
            SearchProblem<PacmanPositionSearchState, PacmanAction> problem) {

        if (problem instanceof PacmanPositionSearchProblem) {
            Coordinate manhat = state.getPacmanLocation();
            Coordinate result = ((PacmanPositionSearchProblem) problem).getGoalLocation();
            double finalResult = (manhat.x - result.x) + (manhat.y - result.y);
            // TODO: implement here
            return finalResult;
        }
        return 0.0;
