...
public class NPuzzleMain {
    static void usage() {
        ...
        out.println("options:");
        out.println(
                "  -n <comma separated numbers> : A list of numbers separated by commas (without space) encoding initial configuration of the tiles");
    ...
    public static void main(String[] args) throws Exception {
         ...
         */
        SearchHeuristic<NPuzzleSearchState, NPuzzleAction> heuristic = (SearchHeuristic) Class.forName(heuristicName)
...
class NPuzzleSearchProblem extends SearchProblem<NPuzzleSearchState, NPuzzleAction> {
    ...
    public NPuzzleSearchState getSuccessor(NPuzzleSearchState state, NPuzzleAction action) {
        ...
        if (newBlankLocation.x < 0 || newBlankLocation.x >= size || newBlankLocation.y < 0
                || newBlankLocation.y >= size)
            throw new IllegalArgumentException("An illegal action " + action + " was provided for the state " + state);

        // Create the successor state
        // (copy all the tiles, and swap the values of the tiles of the old and new
        // blank locations)
